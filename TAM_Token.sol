...
contract MyToken {
  ...
  function tranfer(address receiver, uint256 amount) public returns (bool) {
    require(receiver != address(0), "The receiver address cannot be zero");
    require(_balances[msg.sender] >= amount, "The sender has insufficient balance in the account");
    require(_balances[receiver] + amount > _balances[receiver], "Addition overflow found in receiver account");
  ...
  function tranferFrom(address sender, address receiver, uint256 amount) public returns (bool) {
    require(sender != address(0), "The sender address cannot be zero");
    require(receiver != address(0), "The receiver address cannot be zero");
    require(amount <= _allowance[sender][receiver], "Allownce is not enough");
    require(_balances[sender] >= amount, "The sender has insufficient balance in the account");
    require(_balances[receiver] + amount > _balances[receiver], "Addition overflow found in receiver account");
